...
* user-config
  #+BEGIN_SRC emacs-lisp
    ...
    ;; Personal function: blog nconf show-startup-page open-init-file open-init-sys load-init indent-buffer
    ;; Personal key-bingding: <f1><f2><f3><f4><f5><f12> / C-x C-r / C-s M-n M-p / M-C-7 M-C-8 M-C-9 / C-.
    (use-package init-site)
    ;; 启动页面
